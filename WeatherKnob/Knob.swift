...
// Updated by Jacqueline for Swift 3
//
import UIKit
...
public class Knob: UIControl {
    ...
    private func updateLayer() {
        ...
        indicatorLayer.lineWidth = shapeLayer.lineWidth + 2;
    ...
    override public func continueTracking(_ touch: UITouch, with event: UIEvent?) -> Bool {
        ...
        //angle = (angle + vector.angleFromVector(vector: lastVector)) % (2 * M_PI)
        angle = (angle + vector.angleFromVector(vector: lastVector)).truncatingRemainder(dividingBy: 2 * M_PI)
